...
/**
 * Picks default payment method to pre-select when checkout init data loads.
 *
 * - type: First available non-wallet method (e.g. "cod" or "epayment"). We skip
 *   "wallet" as the default so the user explicitly opts in to using wallet.
 * - epaymentMethodId: If the default type is "epayment", we also pick the first
 *   gateway option (e.g. first card/epayment method id) so the UI has a valid
 *   selection. Otherwise null.
 *
 * Used once when init response arrives to seed selectedPaymentMethodType and
 * selectedEpaymentMethodId in the checkout page.
 */
...
/** Payment result page: derive orderId, success, failed, message from URL + payment-status API. */
